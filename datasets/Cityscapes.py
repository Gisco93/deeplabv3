...
def toPil(mask, categorie=False):
...
if __name__ == "__main__":
    ...
    dataset = Cityscape("../../deep_ml_data", split="fine", scale_size=(1024, 512),  random_flip=True)
    ...
    '''
    ref_img = dataset[0][0].size()
    ref_mask = dataset[0][1].size()
    print(ref_img)
    print(ref_mask)
    ...
    for img,mask, path in dataset:
        if img.size() != ref_img or mask.size() != ref_mask:
            false_imgs.append(path)
            print(path)
