...
# Class for the Vistas dataset
# [LINK]

#Vista images have size 4:3....sometimes
...
class Vistas(Dataset):
    ...
    def __init__(self, path, split="train", subset=None, img_transforms=None, mask_transforms=None,
        ...
        print(self.path_train_img.exists())
        print(self.path_train_gt.exists())
        print(self.path_val_img.exists())
        print(self.path_val_gt.exists())
...
if __name__ == "__main__":
    ...
    dataset = Vistas("../../deep_ml_data", split="train")

    var = torch.tensor([0,0,0]).float()
    mean_tensor = torch.tensor(dataset_stats["mean"]).unsqueeze(1)

    for [img, mask ] in tqdm(dataset):
        img = img.view(3,-1)
        var += torch.sum((img - mean_tensor)**2, 1)/img.size(1)
