...
class DeepLabV3Plus(nn.Module):
    ...
    def forward(self, x):
        ...
        dec = F.upsample(enc2, (lowfeat.size(2), lowfeat.size(3)), mode='bilinear', align_corners=True)
        dec = torch.cat([dec, lowfeat], 1)

        final = self.final(dec)
        final = F.upsample(final, (x.size(2), x.size(3)), mode='bilinear', align_corners=True)
